...
func TestLrem(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("Lrem failed %v", err.Error())
	} else if num != 2 {
		t.Fatalf("Lrem failed, got %d, expected 2", num)
	}
	...
	if err != nil {
		...
		t.Fatalf("Llen failed, got %v, expected 1", length)
	}
	client.Del("l")
}
...
func BenchmarkZadd(b *testing.B) {
	for i := 0; i < b.N; i++ {
		...
		client.Zadd("zrs", "hi"+strconv.Itoa(i), float64(i))
	}
	client.Del("zrs")
}
