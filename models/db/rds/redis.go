...
type Client struct {
	...
	Db          uint          //数据库
	Password    string        //密码
	MaxPoolSize uint          //最大连接池大小
	pool        chan net.Conn //连接通道 //the connection pool
}
...
func readAll(r io.Reader, size int64) ([]byte, error) {
	...
	for {
		m, e := r.Read(data[len(data):cap(data)])
	...
	return data, nil // err is EOF, so return nil explicitly
}
...
func readBulk(reader *bufio.Reader, head string) ([]byte, error) {
	...
	case '$':
		...
		data, err = readAll(lr, int64(size)) // data, err = ioutil.ReadAll(lr)
		if err == nil {
			if len(data) != size {
				err = fmt.Errorf("readAll: not read expected size of bytes")
			} else {
				// read end of line
...
func commandBytesToByteBuffer(cmdbuf *bytes.Buffer, cmd string, args ...string) []byte {
...
func writeRequest(writer io.Writer, cmd string, args ...string) error {
	buf := util.AcquireByteBuffer()
	b := commandBytesToByteBuffer(buf, cmd, args...)
	defer util.ReleaseByteBuffer(buf)
	_, err := writer.Write(b)
	return err
}

func (client *Client) ResetCommandBuffer(cmdbuf *bytes.Buffer, cmd string, argc int) {
	cmdbuf.Reset()
	fmt.Fprintf(cmdbuf, "*%d\r\n$%d\r\n%s\r\n", argc+1, len(cmd), cmd)
}

func (client *Client) AppendCommandBufferWithString(cmdbuf *bytes.Buffer, s string) {
	fmt.Fprintf(cmdbuf, "$%d\r\n%s\r\n", len(s), s)
}

func (client *Client) AppendCommandBufferWithBytes(cmdbuf *bytes.Buffer, b []byte) {
	fmt.Fprintf(cmdbuf, "$%d\r\n%s\r\n", len(b), b)
}

func (client *Client) SendCommandBuffer(cmdbuf *bytes.Buffer) error {
	redisStatusMu.RLock()
	if !redisStatus {
		redisStatusMu.RUnlock()
		return fmt.Errorf("Redis status error!")
	}
	redisStatusMu.RUnlock()
	...
	_, err = client.rawSend(c, cmdbuf.Bytes())
	if err != nil {
...
func (client *Client) sendCommand(cmd string, args ...string) (data interface{}, err error) {
	...
	if !redisStatus {
		redisStatusMu.RUnlock()
		return nil, fmt.Errorf("Redis status error!")
	}
	redisStatusMu.RUnlock()
...
func (client *Client) sendCommands(cmdArgs <-chan []string, data chan<- interface{}) (err error) {
	// // grab a connection from the poolf
	// c, err := client.popCon()
	...
	if err == io.EOF {
		...
		if pong != "PONG" {
			err = RedisError("Unexpected response to PING.")
			goto End
		}
	}
...
func (client *Client) SendCommand(cmd string, args ...string) error {
	if _, err := client.sendCommand(cmd, args...); err != nil {
		return err
	}
	return nil
}
...
func init() {
	go resetRedisStatusRoutine()
}

var errTimes int = 0
var redisStatus bool = true
var redisStatusMu sync.RWMutex //读写锁

func resetRedisStatusRoutine() {
...
func checkRedisStatus(err error) {
	if err != nil {
		redisStatusMu.Lock()
		errTimes++
		if errTimes >= 5 {
...
func (client *Client) popCon() (net.Conn, error) {
	...
	if c == nil {
		...
		c, err = client.openConnection()
		if err != nil {
			log.Warnf("openConnection: %s", err.Error())
			checkRedisStatus(err)
			return nil, err
		}
	}
	return c, nil
...
// sorted set(有序集合) commands

func (client *Client) Zadd2(key string, mapping map[string]float64) (bool, error) {
	args := make([]string, len(mapping)*2+1)
	args[0] = key
	i := 1
	for value, score := range mapping {
		args[i] = strconv.FormatFloat(score, 'f', -1, 64)
		args[i+1] = value
		i += 2
	}
	...
	return res.(int64) == int64(len(mapping)), nil
}

func (client *Client) Zadd(key string, value string, score float64) (bool, error) {
	res, err := client.sendCommand("ZADD", key, strconv.FormatFloat(score, 'f', -1, 64), value)
	if err != nil {
		return false, err
	}
...
func (client *Client) Zrem(key string, values ...string) (bool, error) {
	newFields := []string{key}
	newFields = append(newFields, values...)
	res, err := client.sendCommand("ZREM", newFields...)
	if err != nil {
		return false, err
	}
...
func (client *Client) Zincrby(key string, value string, score float64) (float64, error) {
	res, err := client.sendCommand("ZINCRBY", key, strconv.FormatFloat(score, 'f', -1, 64), value)
	if err != nil {
		return 0, err
	}
...
func (client *Client) Zrank(key string, value string) (int, error) {
	res, err := client.sendCommand("ZRANK", key, value)
	if err != nil {
		return 0, err
	}
...
func (client *Client) Zrevrank(key string, value string) (int, error) {
	res, err := client.sendCommand("ZREVRANK", key, value)
	if err != nil {
		return 0, err
	}
...
func (client *Client) Zscore(key string, member string) (float64, error) {
...
func (client *Client) Del(keys ...string) (bool, error) {
	num := len(keys)

	res, err := client.sendCommand("DEL", keys...)

	if err != nil {
		return false, err
	}

	return res.(int64) == int64(num), nil
}
...
func (client *Client) Hdel(key string, fields ...string) (bool, error) {
	...
	res, err := client.sendCommand("HDEL", newFields...)
...
func (client *Client) Eval(script string, numKeys int, keys ...string) (interface{}, error) {
	newFields := []string{script, strconv.Itoa(numKeys)}
	newFields = append(newFields, keys...)
	res, err := client.sendCommand("EVAL", newFields...)
	return res, err
}
